...
primary_private_fqdn = sys.argv[7]
...
if custom_domain == "true":
    ...
    config = {"_id": replica_set_name, "members": [{ "_id": 0, "host": primary_private_fqdn+":27017", "priority": 1000 }]}
    secondary_nodes = []
    for reservation in data['Reservations']:
        secondary_private_fqdn = reservation["Instances"][0]["PrivateDnsName"]
        tags = reservation["Instances"][0]["Tags"]
        for tag in tags:
            if tag["Key"] == "Name":
                node_index = tag["Value"][-1]
                config["members"].append({"_id": int(node_index), "host": secondary_private_fqdn+":27017", "priority": 0.5})
                secondary_nodes.append([secondary_private_fqdn, False])
    allPassed = False   
